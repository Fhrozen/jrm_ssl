...
IMP_PREFIX = '/export/db01/Local360d/data/TSP/microcone/impulses/imp'
DBPREFIX = 'microcone'
FILEPREFIX = '/export/db01/MICROCONE_MULTI'
...
def preparedata(_ANGLE):
    ...
    while (datasize > actual_size):  
        ...
        if radius_idx >= len(radius_vals): radius_idx=0
        ...
        if np.isinf(indata).any() or np.isinf(outdata).any():
            ...
            _tmp_angle = np.ones((tmp_input.shape[0],2)) * _ANGLE #for allocating the angle and the number of frames related to the silence
            for lg in range (tmp_input.shape[0]):
                a = np.histogram(rms_tmp[lg], bins=[0,120,130])
                if a[0][1] > 13:
                    tmp_target[lg] = 360
                _tmp_angle[lg,1] = (frames-a[0][1])/frames
            size = tmp_input.shape[0]
            tmp_angle = angle_to_image(_tmp_angle, radius_vals[radius_idx], PPM, ROOM_DIMS)
            ...
            for conns in range(iters):
                ...
                with h5py.File(fl_name, 'a') as f:
                    ...
                    ds = f.create_dataset('arr_3', data = radius_vals[radius_idx])
                    ds = f.create_dataset('arr_4', data = tmp_angle[conns])   
